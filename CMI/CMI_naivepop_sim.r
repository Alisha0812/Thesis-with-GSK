...
mu.ind.v2 = log10(array(c(1000,1000,
                          1000,1200,
                          1000,1800,
                          1000,2500,
                          1000,3000,
                          ...
                          1000,6000,
                          2000,3000,
                          2000,4000),dim=c(2,9)))  # V2 Freq_ind, for different scenarios
...
for (i in 1:length(sd.e.vec)){
  for (j in 1:length(z.corr.vec)){
    p3<- pow_graph(data=pow.res2[which(pow.res2$sd.e==sd.e.vec[i] & pow.res2$z.corr==z.corr.vec[j]),],
                   Title1 = paste0("Test for vaccine effect ratio (B/A) - Naive population"),subtitle1 = paste0("SD =",sd.e.vec[i], " & correlation=",z.corr.spec[j]) )
    doc3 <- body_add_gg(doc3, value =p3 , style = "centered" )
  }
}

print(doc3, target  = "power_graph_naive.docx")

save.image("all_naive.RData")
...
#write_csv(pow.res2, "pow.res2.csv")
#write_csv(est.res1, "est.res1.csv")
